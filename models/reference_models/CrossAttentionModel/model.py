...
class CrossAttentionModel(BaseModel):
    ...
    def __init__(self, embedding_layer, prob_and_entr_embedding_layer,   learnable_embedding, learnable_embedding_prob_entropy, cross_attention_layer, pooling_layer, final_layers, initialize_optimizer,
                 device="cuda", ):
        super().__init__()
    ...
    def forward(self, sources, hypotheses, features):
        ...
        h_pe_emb = self.prob_entr_embedding_layer(features["hypotheses_prob_entropy"])
        ...
        extended_hypotheses_mask = torch.concat([torch.ones( batch_size, 1).to("cuda"), hypotheses_mask], dim=-1)
        ...
        for r_tokenized, r_prob_entr in zip(features["references_tokenized"], features["reference_prob_entropy"]):
            ...
            r_pe_emb = self.prob_entr_embedding_layer(r_prob_entr)
            ...
            ref_attention_mask = torch.concat([torch.ones( batch_size, 1).to("cuda"), ref_attention_mask], dim=-1)
