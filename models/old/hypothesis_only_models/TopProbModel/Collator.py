...
class TopProbModelCollator(BaseCollator):
    ...
    def pack_features(self, features):
        ...
        lengths = torch.tensor([c.shape[0] for c in concat_sequence]).long()
        padded_sequence = pad_sequence(concat_sequence, batch_first=True)
        return pack_padded_sequence(padded_sequence, batch_first=True, enforce_sorted=False, lengths=lengths)
