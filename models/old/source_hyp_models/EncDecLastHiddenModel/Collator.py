...
class EncDecLastHiddenCollator:
    '''
    ...
    def __call__(self, batch):
        ...
        with self.tokenizer.as_target_tokenizer():
            ...
            hypothesis_sequence_lengths = torch.tensor([len(x) for x in labels["input_ids"]])

        model_inputs = self.tokenizer(sources, truncation=True,  max_length=self.max_seq_length)

        source_input_lengths = torch.tensor([len(x) for x in model_inputs["input_ids"]])
        ...
        features = {
            "hypothesis_sequence_lengths": hypothesis_sequence_lengths,
            "source_sequence_lengths": source_input_lengths,
            **x_new
        }
