...
from models.old.reference_models.FullDecCometModel.info import FullDecCometModelInfo
...
class FullDecCometModelTrainer:

    def __init__(self, config, smoke_test=False):
        self.config = config
        self.smoke_test = smoke_test
        self.model_info = FullDecCometModelInfo
    ...
    def __call__(self, ):
        ...
        wrapped_nmt_model = NMTWrapper(model_manager.nmt_model, model_manager.tokenizer)

        train_preprocess = self.model_info.preprocess(wrapped_nmt_model, wrapped_model, lookup_table_location=train_path)
        val_preprocess = self.model_info.preprocess(wrapped_nmt_model, wrapped_model, lookup_table_location=val_path)

        train_dataset_preprocessed, train_ref_lookup_table, train_source_lookup_table = train_preprocess(
            train_dataset)
        val_dataset_preprocessed, val_ref_lookup_table, val_source_lookup_table,= val_preprocess(
            validation_dataset)

        # Get the collate functions

        train_collate_fn = self.model_info.collator(model_manager.nmt_model, model_manager.tokenizer, train_source_lookup_table, train_ref_lookup_table, n_references=3)
        val_collate_fn = self.model_info.collator(model_manager.nmt_model,model_manager.tokenizer, val_source_lookup_table, val_ref_lookup_table, n_references=3)
