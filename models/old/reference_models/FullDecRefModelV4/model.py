...
class FullDecRefModelV4(BaseModel):

    def __init__(self, hidden_state_embedding, hidden_state_lstms, prob_entropy_lstm_layer, count_vec_embedding,
        ...
        self.count_vec_embedding = count_vec_embedding
    ...
    def forward(self, sources, hypotheses, features):
        ...
        # Hypothesis count vector embedding

        hypothesis_count_vector_embedding = self.count_vec_embedding(features["hypothesis_tokens"])

        # REFS

        ref_count_embeddings = []
        for ref_tokens in features["reference_tokens"]:
            ref_count_embedded = self.count_vec_embedding(ref_tokens)
            ref_count_embeddings.append(ref_count_embedded)



        features = torch.concat(hidden_states + ref_count_embeddings + [hypothesis_count_vector_embedding], dim=-1)
